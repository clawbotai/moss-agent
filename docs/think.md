...
任务主视图应该收敛成 Claude Web / Codex app 式的单一任务流，而不是多个并列折叠区：

1. 用户需求作为第一条消息。
2. 当前任务补充说明作为用户消息穿插进任务流。
3. 计划、审查、开发、审核等阶段作为 agent 执行块顺序展示。
4. 审查意见直接成为 review / audit 阶段的主输出，不单独拆成一级入口。
5. 交付摘要作为最后一条结果消息直接展示，不再折叠。
6. 原始日志、上下文包、stdout/stderr、token 估算只作为调试详情隐藏在对应阶段内。
7. Artifacts 后续也应该以任务流附件形式出现，而不是新增复杂一级面板。

这样用户只需要理解一条“任务执行故事线”：当前在做什么、刚刚做了什么、哪里出问题、最终交付什么。
